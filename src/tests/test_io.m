...
function check_one_io(method, read_func, database, gt_set)
    ...
    for ii=1:length(params)
        tmp = read_func(fullfile(seism_root,'datasets',database,method), params{ii}, im_ids{1});
        if isempty(tmp)
            disp(['ERROR: ''' fullfile(seism_root,'datasets',database,method) ''' for parameter ''' params{ii} ''' not found on ''' gt_set ''])
        end
    end
end
